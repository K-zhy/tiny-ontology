...
## 规则
- 得到足够信息后立即回答，不要继续深挖
- 学期、共同授课、任课安排问题优先考虑 TeachingAssignment，而不是只查 Course 或 Teacher
- 严格按两行格式输出：第一行 `结论：...`，第二行 `分析：...`
- 必须先给结论，再给简要分析过程
- 不要复述探索过程，不要说“我先查了”或“根据结果”
- 如存在并列，直接在 `结论` 中点名并列对象和关键信息
- 找不到就直接写没找到"""
...
def execute_tool(graph, tool_name: str, inp: dict) -> dict:
    ...
    try:
        if tool_name == "list_object_types":
            ...
            if not types:
                return {"content": "系统中没有注册任何对象类型", "summary": "no types", "data": [], **empty}
            ...
            return {"content": "\n".join(parts), "summary": f"listed {len(types)} object types", "data": types, **empty}
            ...
            if not nodes:
                ...
                return {"content": f"在 {scope} 中未找到匹配 \"{keyword}\" 的对象", "summary": "empty semantic search", "data": [], **empty}
            ...
            return {"content": "\n---\n".join(parts), "summary": f"semantic search '{keyword}' → {len(nodes)} results", "data": nodes, "visited_node_keys": visited, "visited_edges": []}
            ...
            if not nodes:
                return {"content": f"未找到匹配的 {obj_type} 对象（{'模糊' if fuzzy else '精确'}匹配）", "summary": f"empty ({obj_type})", "data": [], **empty}
            visited = [n["_node_key"] for n in nodes]
            ...
            return {"content": "\n---\n".join(parts), "summary": f"found {len(nodes)} {obj_type}", "data": nodes, "visited_node_keys": visited, "visited_edges": []}
            ...
            if not neighbors:
                ...
                return {"content": f"从 {source_node.get('name', node_key)} 沿 '{trav_name}' 遍历无结果。{hint}", "summary": "empty traversal", "data": [], "visited_node_keys": [node_key], "visited_edges": []}
            ...
            return {"content": "\n---\n".join(parts), "summary": f"traversed {node_key} → {len(neighbors)} nodes", "data": neighbors, "visited_node_keys": visited, "visited_edges": edges}
            ...
            return {"content": format_node(graph, node, detail=True), "summary": f"detail for {node_key}", "data": node, "visited_node_keys": [node_key], "visited_edges": []}
            ...
            return {"content": json.dumps(result, ensure_ascii=False), "summary": f"called {inp.get('function_name', '')}", "data": result.get("data", result), "error": result.get("error"), **empty}
            ...
            if result.get("success"):
                reload_graph()
            return {"content": json.dumps(result, ensure_ascii=False), "summary": f"executed {inp.get('action_name', '')}", "data": result, "error": result.get("error"), **empty}
    ...
    for iteration in range(max_iterations):
        ...
        if tool_use_blocks:
            ...
            for tool in tool_use_blocks:
                ...
                if iteration + 1 >= 2:
                    content += f"\n\n[已执行 {iteration + 1} 步。请严格按两行格式回答：第一行 `结论：...`，第二行 `分析：...`。必须先给结论，再简要分析，不要复述探索过程，不要再调工具。]"
        ...
        if tool_call:
            ...
            hint = f"如果数据足够请严格按 `结论：...`、`分析：...` 两行格式回答。还剩 {remaining} 步。" if remaining <= 3 else ""
