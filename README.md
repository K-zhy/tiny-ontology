...
**典型调用场景**：
- 用户问"学号 3001 的学生详细信息" → `get_object_detail(object_type="Student", object_id="3001")`
...
| 函数名 | 参数 | 返回值 | 说明 |
|--------|------|--------|------|
| `getAvgScore` | `studentSno` | 平均分 (REAL) | 计算指定学生的所有成绩平均分。同时是 Student 的派生属性 `avgScore` |
| `getCourseAvgScore` | `courseCno` | 平均分 (REAL) | 计算指定课程的所有学生平均分 |
| `getPassRate` | `courseCno` | 通过率 (TEXT, 如 "75.0%") | 计算指定课程的及格率（>=60分为及格）。同时是 Course 的派生属性 `passRate` |
| `getAllCourseAvgScores` | 无 | 列表 [{Cno, name, semester, avg_score, student_count}] | 获取所有课程的平均分和学生人数汇总 |
| `getTopStudents` | `courseCno`, `limit`(可选) | 列表 [{Sno, name, age, gender, class_name, Sbirthday, score_value}] | 获取指定课程的成绩排名，按分数降序 |
| `searchByName` | `keyword`(可选，为空返回全部) | 列表 [{object_id, name, result_type}] | 跨 Student/Teacher/Course 模糊搜索名称，实现 Nameable 接口 |
...
| Action 名 | 参数 | 说明 |
|-----------|------|------|
| `createScore` | `studentSno`, `courseCno`, `scoreValue`, `examDate` | 录入一条新成绩。会先调 validateScore 校验分数范围（0-100），执行后写入 audit_log |
| `updateScore` | `scoreId`, `scoreValue`, `examDate`(可选) | 修改已有成绩的分数和/或考试日期 |
| `deleteScore` | `scoreId` | 删除一条成绩记录 |
| `assignTeacher` | `courseCno`, `teacherTno` | 为课程新增一条教师授课关系（底层写入 TC，多教师可并存） |
